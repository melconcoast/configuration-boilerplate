...
export default class Notes extends Component {
    ...
    fetchData(){
        return axios.get('be-services/projectManagement/project/'+this.id+'/note').then(result => {this.setState({data: result.data, newNote:  ""}, this.scrollToBottom.bind(this))});
    }
                ...
                <div className="mainContainer">
                    <div id="notesList" className="notesList">
                        {this.state.data.map(elem => {
                            if(elem.isUrgent)
                                return (
                                    <ReactCSSTransitionGroup key={elem.id}
                                        transitionName="fade"
